...
const OrderDetailModal = ({ isOpen, onClose, order, updateOrderStatus }) => {
  ...
  <div className="p-6 m-6 space-y-8">
    {/* Informasi Pesanan */}
    <div>
      <h2 className="text-lg font-semibold text-pink-600 mb-4">Informasi Pesanan</h2>
      <p>
    ...
    {/* Item Pesanan */}
    <div>
      <h2 className="text-lg font-semibold text-pink-600 mb-4">Item Pesanan</h2>
      {order.order_items && order.order_items.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="w-full border border-pink-300 rounded-lg bg-white">
            <thead className="bg-pink-100">
              <tr>
                <th className="py-3 px-4 text-left text-pink-700">Nama Produk</th>
                <th className="py-3 px-4 text-center text-pink-700">Qty</th>
                <th className="py-3 px-4 text-right text-pink-700">Harga</th>
                <th className="py-3 px-4 text-right text-pink-700">Subtotal</th>
              </tr>
            </thead>
            <tbody>
              {order.order_items.map((item) => (
                <tr key={item.id} className="border-t">
                  <td className="py-3 px-4">{item.product.product_name}</td>
    ...
    {/* Informasi Pelanggan */}
    <div>
      <h2 className="text-lg font-semibold text-pink-600 mb-4">Informasi Pelanggan</h2>
      <p>
    ...
    {/* Ubah Status Pesanan */}
    <div>
      <label className="block mb-2 font-semibold text-pink-700">Ubah Status Pesanan</label>
      <select
        value={status}
